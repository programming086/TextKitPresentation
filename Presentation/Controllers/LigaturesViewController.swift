...
class LigaturesViewController: TextViewSlideViewController, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    func pickerView(pickerView: UIPickerView, attributedTitleForRow row: Int, forComponent component: Int) -> NSAttributedString? {
        ...
        let attributes = [NSFontAttributeName : normalFont, NSForegroundColorAttributeName : UIColor.whiteColor()];
        return NSAttributedString(string: "\(options[row])", attributes: attributes)
    }
    
    func pickerView(pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        textStorage.addAttribute(NSLigatureAttributeName, value: row, range: NSMakeRange(0, textStorage.length))
    }
}
